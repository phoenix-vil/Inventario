...
# -*- coding: utf-8 -*-
"""
[QA] Revierte imprimirTicket()/imprimirDetalle() a su version original
(HTML propio con el diseno que si gustaba), deshaciendo el cambio
anterior que las hacia abrir el PDF de jsPDF por error.
Uso: cd ~/inventario-qa/static && python3 qa_revertir_imprimir.py
"""
import os, re

STATIC = os.path.expanduser('~/inventario-qa/static')

viejo_pagos = '''async function imprimirTicket(){
...
nuevo_pagos = '''function imprimirTicket(){
  if(!ticketActual)return;
...
if n1 == 1:
    ...
    print("OK pagos.html: imprimirTicket() revertida a su version original")
elif 'win.document.write(`<html><head><title>Punto de venta' in src:
    print("* pagos.html: ya estaba revertido")
else:
    print("ERROR pagos.html: no se encontro el bloque exacto (coincidencias: " + str(n1) + ")")

viejo_hist = '''async function imprimirDetalle(){
...
nuevo_hist = '''function imprimirDetalle(){
  if(!ventaDetalle)return;
  const win=window.open('','_blank');
...
if n2 == 1:
    ...
    print("OK historial.html: imprimirDetalle() revertida a su version original")
elif 'win.document.write(`<html><head><title>Historial' in src2:
    print("* historial.html: ya estaba revertido")
else:
...
if ok_total:
    ...
    print("Listo. Imprimir vuelve a mostrar su propio diseno (el que te gustaba),")
    print("independiente del PDF de Descargar/WhatsApp.")
else:
