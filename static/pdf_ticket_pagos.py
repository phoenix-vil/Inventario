...
# -*- coding: utf-8 -*-
"""
Rediseña el ticket de venta compartido por WhatsApp en pagos.html:
logo real, formato de tabla limpio, PDF compartible (en vez de texto).
Uso: cd ~/inventario/static && python3 pdf_ticket_pagos.py
"""
import os, re
...
# ================================================================
# 2. Agregar las funciones de PDF (logo, cargarImagenBase64, etc.)
# ================================================================
...
'''
    marcador2 = 'function generarTicketHTML(v){'
    if marcador2 in src:
        src = src.replace(marcador2, funciones + marcador2, 1)
        cambios.append('funciones de PDF agregadas')
    else:
        print("ERROR: no se encontro 'function generarTicketHTML(v){'")
else:
...
viejo = '''function compartirWhatsApp(){
  const txt=ticketTextoPlano(ticketActual);
  if(!txt)return;
...
nuevo = '''function compartirWhatsApp(){
  ...
  compartirComoPDFVenta(ticketActual, 'ticket_'+ticketActual.id+'.pdf');
}'''
...
elif 'compartirComoPDFVenta(ticketActual' in src:
...
if ok:
    ...
    print("Listo. El ticket de venta en Punto de Venta ahora comparte un PDF con logo.")
else:
